...
def main():
    ...
    if X is None or len(X) == 0: 
        print("[ERROR] No se encontraron archivos de SisFall. Revisa DATASET_PATH.")
        return

    # Balancear clases de forma robusta
    ...
    # Diccionario unificado usando las claves de tu proyecto original ('arbol', 'rf', 'mlp')
    modelos = {
        "arbol": DecisionTreeClassifier(max_depth=8, min_samples_leaf=2, class_weight="balanced", random_state=RANDOM_STATE),
        "rf": RandomForestClassifier(n_estimators=100, max_depth=10, min_samples_leaf=2, class_weight="balanced", random_state=RANDOM_STATE),
    ...
    for nombre, modelo in modelos.items():
        ...
        print(f"  Modelo {nombre:<5}: {acc * 100:.2f}%")

    joblib.dump({"modelos": modelos, "scaler": scaler, "nombres_clases": NOMBRES_CLASES}, "modelos_sisfall_sentado.pkl")
    print("\n[INFO] Modelos guardados correctamente en: modelos_sisfall_sentado.pkl")
