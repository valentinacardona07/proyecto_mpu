...
def recortar_sentado(df, inicio=0.40, fin=0.60):
    n = len(df)
    i0 = int(inicio * n)
    i1 = int(fin * n)
    if i1 - i0 < 50:
...
def evaluar_modelo(nombre, y_test, y_pred):
    ...
    print(classification_report(y_test, y_pred, labels=labels, target_names=NOMBRES_CLASES, zero_division=0))
...
def main():
    ...
    grupos_train, grupos_test = grupos[train_idx], grupos[test_idx]
    ...
    accuracies, matrices, nombres, modelos_entrenados = [], [], [], {}
